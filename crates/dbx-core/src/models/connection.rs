...
mod tests {
    ...
    #[test]
    fn historical_sqlserver_disabled_encryption_setting_stays_on_native_driver() {
        ...
        assert_eq!(
            canonical.url_params.as_deref(),
            Some("applicationName={DBX; Client};password=50%;sqlserverEncryption=disabled;encrypt=false")
        );
    }

    #[test]
    fn sqlserver_disabled_encryption_canonicalization_preserves_escaped_braces() {
        ...
        config.driver_profile = Some("sqlserver".to_string());
        config.driver_label = Some("SQL Server".to_string());
        config.url_params = Some(params.to_string());

        let canonical = config.canonicalized();

        assert_eq!(canonical.driver_profile.as_deref(), Some("sqlserver"));
        assert_eq!(canonical.url_params.as_deref(), Some(params));
    }
