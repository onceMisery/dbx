...
async function loadRemoteDdl(request: ObjectDdlRequest, cacheKey: string, force: boolean): Promise<string> {
  ...
  const entry: InFlightDdlLoad = { force, invalidated: false, generation: currentGeneration(), promise: Promise.resolve("") };
...
export async function loadObjectDdl(request: ObjectDdlRequest, options?: { force?: boolean }): Promise<ObjectDdlLoadResult> {
  ...
  if (!options?.force) {
    ...
    if (generation !== currentGeneration()) return { ddl: await loadRemoteDdl(request, cacheKey, false), cacheStatus: "remote" };
    if (cached !== null) {
