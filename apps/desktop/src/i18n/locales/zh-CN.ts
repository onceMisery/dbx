...
export default withEnglishFallback({
  ...
  connection: {
    ...
    sqlServerLegacyCompatibilityComponent: "SQL Server TLS 1.0 兼容组件",
    sqlServerDriverModeAuto: "自动",
    sqlServerDriverModeTls10: "TLS 1.0",
    sqlServerTransportModeHint:
      "自动模式使用内置驱动，并可为旧版服务端降级到仅登录阶段加密或不加密传输。TLS 1.0 模式使用 Java 兼容组件，默认设置为 encrypt=true、trustServerCertificate=true、sslProtocol=TLSv1；用户明确指定的高级参数优先。此独立兼容进程会在启动时启用旧版 TLS 算法，因此所有会话都共享放宽后的安全策略，包括明确请求 TLSv1.2 或 encrypt=false 的会话。此模式不能解决凭据错误或 SQL Server 18456 错误。仅应在可信网络、VPN 或 SSH 隧道中使用该组件。",
