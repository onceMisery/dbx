...
- 第一期以多库执行弹窗作为目标组的唯一创建、编辑和复用入口。用户在这里选择完整目标后即可保存，流程最贴近“当前 SQL 要执行到哪些库”的上下文，也能在保存前立即完成同类型校验和 database/schema 校验。
- 连接界面暂不新增另一套“schema 连接分组”保存模型。连接界面现有的连接分组继续只负责连接导航和展示，不直接代表 SQL 执行范围。
- 后续如果需要从连接界面快速创建目标组，入口名称应使用“创建 SQL 执行目标组”或“添加到 SQL 执行目标组”，并跳转或复用目标组编辑器，而不是直接把选中的连接分组保存为执行组。
- 连接界面快捷入口必须以完整的 `connectionId + catalog + database + schema` 作为保存单位：如果用户只选中了连接，没有明确选择 database/schema，则必须继续打开目标选择器补全目标；不得默认把该连接下所有 database/schema 加入执行范围。
- 快捷入口只允许选择同一种有效数据库类型的目标；跨类型选择时应提示用户拆分为多个目标组。创建或更新成功后，连接界面和多库执行弹窗读取同一个目标组 Store，目标组的重命名、失效校验、另存为和删除规则保持一致。
- 从侧边栏连接分组批量生成目标组（如未来提供）只能生成一次性的目标快照，必须让用户确认实际 database/schema 成员后再保存；侧边栏分组后续增删连接不会自动修改已经保存的目标组。

因此，“SQL 执行目标组”是面向执行语义的稳定预设；“侧边栏连接分组”是面向导航语义的动态展示集合。两者可以提供快捷转换，但不共享名称、成员状态或生命周期。
